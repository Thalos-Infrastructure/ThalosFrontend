...
describe("connect profile API", () => {
  ...
  it("loads the authenticated editor profile from /profiles/me", async () => {
    ...
    await expect(getOwnConnectProfile("jwt")).resolves.toEqual({
      success: true,
      data: bothTypes,
    })
  ...
  it("saves both additive profile types through Nest", async () => {
    ...
    const result = await updateOwnConnectProfile(bothTypes, "jwt")
  ...
  it("unwraps the { profile } envelope on load and save", async () => {
    ...
    await expect(updateOwnConnectProfile(bothTypes, "jwt")).resolves.toEqual({
      success: true,
      data: bothTypes,
    })
  })
