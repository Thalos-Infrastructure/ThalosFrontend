...
const routeCases: RouteCase[] = [
  ...
  },
  {
    ...
    invoke: () => migration.getEscrowsByRole({
      address: "G-PROVIDER",
  ...
  },
  {
    ...
    invoke: () => migration.approveMilestone(
      "contract-1", "0", "G-APPROVER", "multi-release", token,
    ),
  ...
  },
  {
    ...
    invoke: () => migration.changeMilestoneStatus(
      "contract-1", "0", "ipfs://evidence", "completed", "G-PROVIDER",
      "multi-release", token,
    ),
    ...
    trustlessWorkArgs: [
      "contract-1", "0", "ipfs://evidence", "completed", "G-PROVIDER", "multi-release",
    ],
  },
  {
    operation: "releaseFunds",
    invoke: () => migration.releaseFunds(
      "contract-1", "G-RELEASER", "multi-release", "0", token,
    ),
...
describe("escrow migration routing", () => {
  ...
  it.each(routeCases)(
    ...
    async ({ operation, invoke, nestMock, trustlessWorkMock, trustlessWorkArgs }) => {
      ...
      expect(mocks.emitTelemetry).toHaveBeenCalledWith(expect.objectContaining({
        operation,
        path: "trustless_work",
        outcome: "success",
      }));
  ...
  it.each([
    ...
    const selectedMock = enabled
      ? mocks.api.buildFundEscrow
      : mocks.trustlessWork.fundEscrow;
    selectedMock.mockResolvedValue({ success: false, error: "route unavailable" });

    const result = await migration.fundEscrow(
      "contract-1", "G-SIGNER", 25, "single-release", token,
    );
