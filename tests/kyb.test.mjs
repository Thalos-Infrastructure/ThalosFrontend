...
test("canStartKybSession requires all CreateKybSessionDto fields", () => {
  assert.equal(canStartKybSession({ business_name: "Acme", registration_number: "123", country: "US", entity_type: "company" }), true)
  assert.equal(canStartKybSession({ business_name: "Acme", registration_number: "123", country: "US", entity_type: null }), false)
  assert.equal(canStartKybSession({ business_name: "Acme", registration_number: "", country: "US", entity_type: "company" }), false)
})

test("buildCreateKybSessionDto trims and submits the valid DTO shape", () => {
  assert.deepEqual(
    buildCreateKybSessionDto("GABC", { business_name: " Acme ", registration_number: " 123 ", country: " US ", entity_type: "startup" }),
    { wallet_address: "GABC", business_name: "Acme", registration_number: "123", country: "US", entity_type: "startup" },
  )
})
