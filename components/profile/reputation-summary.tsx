import { ShieldCheck, Github, Milestone, Handshake, Coins, GitPullRequest } from "lucide-react"
...
export function ReputationSummary({ reputation }: { reputation: ReputationSummary | null }) {
  ...
  return <section className="rounded-2xl border border-border/40 bg-card/50 p-6" aria-label="Reputation">
    <div className="mb-5 flex items-center justify-between"><h3 className="text-lg font-semibold">Reputation</h3>{reputation.githubVerified && <span className="flex items-center gap-1.5 text-sm text-green-400"><Github size={16}/> Verified GitHub</span>}</div>
    ...
    {reputation.valueVisible && reputation.totalValueReleased != null && <div className="mt-5 flex items-center gap-2 border-t border-border/40 pt-4 text-sm"><Coins size={16} className="text-[#f0b400]"/><span>Total value released: <strong>{reputation.totalValueReleased.toLocaleString()} {reputation.totalValueAsset || "USDC"}</strong></span></div>}
  </section>
}
