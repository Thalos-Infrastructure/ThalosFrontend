...
const services = [
  {
    id: "onramp",
    label: "Fiat to USDC On-ramp",
    description: "Convert local currencies to USDC via integrated partners.",
    icon: "arrow-up-right",
  },
  {
    id: "escrow",
    label: "Escrow (Protected Funds)",
    description: "Lock funds in a smart contract until conditions are met.",
    icon: "lock",
  },
  {
    id: "milestones",
    label: "Staged Payments",
    description: "Release funds progressively based on milestone completion.",
    icon: "layers",
  },
  {
    id: "accumulative",
    label: "Accumulative Payments",
    description: "Collect installments toward a target amount over time.",
    icon: "trending-up",
  },
  {
    id: "yield",
    label: "Yield on Idle Funds",
    description: "Earn returns on escrowed funds while they are held.",
    icon: "sparkles",
    optional: true,
  },
]
...
const roles = [
  {
    id: "sender",
    label: "Sender (Payer)",
    description: "The party initiating and funding payments.",
  },
  {
    id: "receiver",
    label: "Receiver (Seller / Provider)",
    description: "The party receiving funds upon conditions met.",
  },
  {
    id: "platform",
    label: "Platform (Optional)",
    description: "An intermediary managing the payment flow.",
    optional: true,
  },
]
...
const paymentLogic = [
  {
    ...
    description: "A single payment released upon condition.",
  },
  {
    id: "milestones",
    label: "Payments by Milestones",
    description: "Funds released incrementally per deliverable.",
  },
  {
    id: "accumulation",
    label: "Monthly Accumulation",
    description: "Monthly contributions until target amount reached.",
  },
]
...
export function PlatformBuilder() {
  ...
  const { displayed: twText, isTyping: twActive } = useTypewriter(t("builder.tag"), isVisible, {
    typeSpeed: 120,
    deleteSpeed: 60,
  ...
  return (
    <section id="builder" className="relative py-28" ref={ref}>
      <div className={cn("mx-auto max-w-4xl px-6 section-reveal", isVisible && "is-visible")}>
        ...
        <div className="mb-10 flex flex-wrap items-center justify-center gap-2">
          {steps.map((step, i) => (
            <div key={step} className="flex items-center gap-2">
              ...
              {i < steps.length - 1 && (
                <div
                  className={cn("h-px w-6", i < currentStep ? "bg-[#f0b400]" : "bg-border/30")}
                />
              )}
            </div>
          ))}
        ...
        <div className="rounded-2xl border border-border/20 bg-card/40 p-6 backdrop-blur-sm shadow-[0_8px_40px_rgba(0,0,0,0.3),0_1px_3px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.06)] md:p-10">
          {currentStep === 0 && (
            <div className="flex flex-col gap-4">
              ...
              <p className="mb-4 text-sm font-medium text-muted-foreground">
                Choose the building blocks for your payment platform.
              </p>
                  ...
                  >
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {service.optional && (
                          <span className="rounded-full bg-card/40 px-2 py-0.5 text-xs font-medium text-muted-foreground">
                            Optional
                          </span>
                        )}
                      </div>
          ...
          {currentStep === 1 && (
            <div className="flex flex-col gap-4">
              ...
              <p className="mb-4 text-sm font-medium text-muted-foreground">
                Define the participants in your payment flow.
              </p>
          ...
          {currentStep === 2 && (
            <div className="flex flex-col gap-4">
              ...
              <p className="mb-4 text-sm font-medium text-muted-foreground">
                Choose how funds are released in your escrow flow.
              </p>
              <div className="flex flex-col gap-3">
                {paymentLogic.map((logic) => (
                  ...
                  >
                    ...
                    >
                      {selectedLogic === logic.id && (
                        <div className="h-2.5 w-2.5 rounded-full bg-[#f0b400]" />
                      )}
                    </div>
                    <div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4">
                {[
                  {
                    title: "Services",
                    items: services.filter((s) => selectedServices.has(s.id)).map((s) => s.label),
                  },
                  {
                    title: "Roles",
                    items: roles.filter((r) => selectedRoles.has(r.id)).map((r) => r.label),
                  },
                  {
                    title: "Payment Logic",
                    items: [paymentLogic.find((l) => l.id === selectedLogic)?.label ?? ""],
                  },
                ].map((group) => (
                  <div
                    key={group.title}
                    className="rounded-xl border border-border/10 bg-card/20 p-5"
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {group.items.map((item) => (
                        <span
                          key={item}
                          className="rounded-full bg-[#f0b400]/10 px-3 py-1 text-xs font-semibold text-[#f0b400]"
                        >
                          {item}
                        </span>
                      ))}
              ...
              <div className="rounded-xl border border-border/10 bg-card/20 p-5">
                ...
                <div className="flex flex-wrap items-center justify-center gap-3">
                  {[
                    ...
                    "USDC",
                    "Escrow",
                    selectedLogic === "milestones"
                      ? "Milestone Release"
                      : selectedLogic === "accumulation"
                        ? "Accumulation"
                        : "Single Release",
                    "Receiver",
                  ].map((node, i, arr) => (
                    <div key={node} className="flex items-center gap-3">
                      <span className="rounded-lg bg-[#f0b400]/10 px-3 py-1.5 text-xs font-semibold text-[#f0b400]">
                        {node}
                      </span>
                      {i < arr.length - 1 && (
                        <svg
