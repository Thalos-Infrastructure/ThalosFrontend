            ...
            <div key={step} className="flex items-center gap-2">
              <button
                ...
                className={cn(
                  ...
                  i === currentStep
                    ? "bg-[#d4a843] text-background shadow-[0_4px_20px_rgba(212,168,67,0.3)]"
                    : i < currentStep
              ...
              {i < steps.length - 1 && (
                <div className={cn("h-px w-6", i < currentStep ? "bg-[#d4a843]" : "bg-border/30")} />
              )}
            </div>
          ))}
        </div>
                ...
                {services.map((service) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedServices.has(service.id)
                        ? "border-[#d4a843]/25 bg-[#d4a843]/8 shadow-[0_4px_16px_rgba(212,168,67,0.12),0_1px_2px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.06)]"
                  ...
                  >
                    <div className={cn(
                      ...
                      selectedServices.has(service.id) ? "bg-[#d4a843]/15 text-[#d4a843]" : "bg-card/40 text-muted-foreground"
                    )}>
                ...
                {roles.map((role) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      selectedRoles.has(role.id) ? "bg-[#d4a843]/15 text-[#d4a843]" : "bg-card/40 text-muted-foreground"
                    )}>
                ...
                {paymentLogic.map((logic) => (
                  ...
                  >
                    <div className={cn(
                      ...
                      selectedLogic === logic.id ? "border-[#d4a843]" : "border-muted-foreground/40"
                    )}>
                      {selectedLogic === logic.id && <div className="h-2.5 w-2.5 rounded-full bg-[#d4a843]" />}
                    </div>
                    <div>
                  ...
                  <div key={group.title} className="rounded-xl border border-border/10 bg-card/20 p-5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {group.items.map((item) => (
                        <span key={item} className="rounded-full bg-[#d4a843]/10 px-3 py-1 text-xs font-semibold text-[#d4a843]">{item}</span>
                      ))}
                    </div>
                  </div>
                ))}
                ...
                <div className="flex flex-wrap items-center justify-center gap-3">
                  {["Sender", "Fiat On-ramp", "USDC", "Escrow", selectedLogic === "milestones" ? "Milestone Release" : selectedLogic === "accumulation" ? "Accumulation" : "Single Release", "Receiver"].map((node, i, arr) => (
                    <div key={node} className="flex items-center gap-3">
                      <span className="rounded-lg bg-[#d4a843]/10 px-3 py-1.5 text-xs font-semibold text-[#d4a843]">{node}</span>
                      {i < arr.length - 1 && (
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[#d4a843]/30">
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            {currentStep < steps.length - 1 ? (
              <Button
                ...
                className="rounded-full bg-[#d4a843] text-background font-semibold shadow-[0_4px_16px_rgba(212,168,67,0.25),0_1px_3px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.2)] hover:bg-[#b0c4de] hover:text-background hover:shadow-[0_4px_20px_rgba(176,196,222,0.35),0_1px_3px_rgba(0,0,0,0.4)] transition-all duration-400"
              >
                Next Step
              </Button>
            ) : (
              <Button className="rounded-full bg-[#d4a843] text-background font-semibold shadow-[0_4px_16px_rgba(212,168,67,0.25),0_1px_3px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.2)] hover:bg-[#b0c4de] hover:text-background hover:shadow-[0_4px_20px_rgba(176,196,222,0.35),0_1px_3px_rgba(0,0,0,0.4)] transition-all duration-400">
                Create Payment Flow
              </Button>
            )}
          </div>
