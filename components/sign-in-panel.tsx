...
export function SignInPanel({ open, onClose }: SignInPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative z-10 flex w-full max-w-4xl overflow-hidden rounded-2xl border border-border/20 bg-card shadow-[0_40px_120px_rgba(0,0,0,0.3)]"
      ...
      >
        ...
        <div className="relative flex flex-1 flex-col justify-center overflow-y-auto px-8 py-10 md:px-10">
          ...
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-muted-foreground">{t("signin.desc")}</p>
          </div>

          <div className="mb-6">
            <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{t("signin.accountType")}</p>
              ...
              {([
                ...
                <button key={type.id} onClick={() => setProfileType(type.id)}
                  className={cn("flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold transition-all duration-300",
                    profileType === type.id ? "border-[#f0b400]/30 bg-[#f0b400]/8 text-[#f0b400]" : "border-border/30 bg-secondary/50 text-muted-foreground hover:border-border hover:text-foreground"
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/40">{t("signin.or")}</span>
            <div className="h-px flex-1 bg-border/30" />
          </div>
          ...
          <div className="mt-7 flex items-center justify-between border-t border-border/20 pt-4">
            <Link href="/admin" onClick={onClose}>
              <button className="flex items-center gap-2 text-xs font-medium text-muted-foreground/50 hover:text-[#f0b400] transition-colors">
            ...
            <p className="text-[10px] text-muted-foreground/30">{t("signin.secured")}</p>
          </div>
        </div>
      </div>
    </div>
