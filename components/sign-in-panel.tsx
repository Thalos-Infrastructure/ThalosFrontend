...
export function SignInPanel({ open, onClose }: SignInPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" onClick={onClose}>
      ...
      {/* Wide popup card */}
      <div
        className="relative z-10 flex w-full max-w-[900px] overflow-hidden rounded-2xl border border-white/[0.06] bg-[#0c0c0e] shadow-[0_40px_120px_rgba(0,0,0,0.7),0_0_0_1px_rgba(255,255,255,0.03)]"
      ...
      >
        ...
        <div className="relative hidden w-[420px] shrink-0 overflow-hidden md:block">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-8">
            <p className="text-lg font-bold text-white/90 leading-snug text-balance">
              Protected payments. Productive capital.
            </p>
            <p className="mt-2 text-xs text-white/40 font-medium">
              Escrow infrastructure on Stellar
            </p>
          </div>
        </div>

        {/* Right side - Sign In form */}
        <div className="relative flex flex-1 flex-col justify-center px-8 py-10 md:px-10">
          {/* Close button */}
          <button
            onClick={onClose}
            className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full text-white/30 transition-colors hover:bg-white/8 hover:text-white"
            aria-label="Close"
          >
                ...
                <button
                  ...
                  className={cn(
                    ...
                    profileType === type.id
                      ...
                      : "border-white/[0.06] bg-white/[0.02] text-white/40 hover:border-white/12 hover:text-white/70"
                  )}
                >
                  {type.icon}
                  {type.label}
          ...
          <div className="flex flex-col gap-2.5">
            <Link href={dashboardHref} onClick={onClose}>
              <Button variant="outline" className="h-11 w-full gap-3 rounded-xl border-white/[0.06] bg-white/[0.02] text-sm text-foreground font-semibold hover:bg-white/[0.06] hover:text-white hover:border-white/12 transition-all duration-300">
