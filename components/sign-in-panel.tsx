...
export function SignInPanel({ open, onClose }: SignInPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center" onClick={onClose}>
      ...
      {/* Card */}
      <div
        className="relative z-10 w-full max-w-md mx-4 rounded-2xl border border-white/8 bg-background/95 shadow-[0_32px_100px_rgba(0,0,0,0.6),0_0_0_1px_rgba(255,255,255,0.04)] backdrop-blur-xl"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground/50 transition-all duration-300 hover:bg-white/8 hover:text-foreground"
          aria-label="Close"
        >
        ...
        <div className="px-8 py-10">
          ...
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground/60">{t("signin.desc")}</p>
          </div>

          {/* Profile type */}
          <div className="mb-7">
            <p className="mb-2.5 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/40">{t("signin.accountType")}</p>
          ...
          <div className="flex flex-col gap-2.5">
            <Link href={dashboardHref} onClick={onClose}>
              <Button variant="outline" className="h-11 w-full gap-3 rounded-xl border-white/8 bg-white/3 text-sm text-foreground font-semibold hover:bg-white/8 hover:text-white hover:border-white/15 transition-all duration-300">
          ...
          <div className="my-5 flex items-center gap-3">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/25">{t("signin.or")}</span>
            <div className="h-px flex-1 bg-white/6" />
          </div>
          ...
          <div className="mt-8 flex items-center justify-between border-t border-white/5 pt-4">
            <Link href="/admin" onClick={onClose}>
              <button className="flex items-center gap-2 text-xs font-medium text-muted-foreground/35 hover:text-[#f0b400] transition-colors">
