...
export function BottomBar({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  useEffect(() => {
    if (!initialized && barRef.current) {
      ...
      setPos({ x: (window.innerWidth - barW) / 2, y: window.innerHeight - 72 })
      setInitialized(true)
    }
  }, [initialized])
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (menuRef.current && !menuRef.current.contains(e.target as Node) && barRef.current && !barRef.current.contains(e.target as Node)) {
        setShowMenu(false)
      }
    }
  ...
  const onPointerMove = useCallback((e: React.PointerEvent) => {
    ...
    const newX = Math.max(0, Math.min(window.innerWidth - 320, dragStart.current.px + dx))
    ...
    <>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-background/70 backdrop-blur-xl" onClick={() => setShowSignIn(false)}>
          ...
          >
            ...
            <div className="mb-8 text-center">
              ...
              <p className="mt-2 text-sm font-medium text-muted-foreground">Choose your profile type and start assembling your payment platform.</p>
            </div>
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-2">
                {([{id: "personal", label: "Personal / Retail"}, {id: "business", label: "Business / Enterprise"}] as const).map((type) => (
                  <button key={type.id} onClick={() => setProfileType(type.id)}
                    className={cn("rounded-xl border p-3 text-sm font-semibold transition-all duration-400",
                      profileType === type.id
                        ? "border-[#e6b800]/25 bg-[#e6b800]/10 text-[#e6b800]"
            ...
            <div className="flex flex-col gap-3">
              <Link href={profileType === "personal" ? "/dashboard/personal" : "/dashboard/business"} onClick={() => setShowSignIn(false)}>
                <Button variant="outline" className="h-11 w-full gap-3 rounded-xl border-border/20 bg-card/30 text-foreground font-semibold shadow-[0_2px_8px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.04)] hover:bg-[#b0c4de]/10 hover:text-[#b0c4de] hover:border-[#b0c4de]/30 transition-all duration-400">
            ...
            <p className="mt-3 text-center text-xs font-medium text-muted-foreground">Securely connect your wallet to access escrow features.</p>
          </div>
        </div>
      )}
      ...
      {showQR && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-background/80 backdrop-blur-xl" onClick={() => setShowQR(false)}>
          <div
            className="relative mx-4 w-full max-w-sm rounded-2xl border border-border/20 bg-card p-10 shadow-[0_24px_80px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.06)]"
            onClick={(e) => e.stopPropagation()}
          >
            <button
            ...
            <p className="mb-6 text-center text-sm font-bold text-foreground">
              Scan to access Thalos Mobile
            </p>

            {/* Traditional QR code pattern */}
            <div className="mx-auto flex h-56 w-56 items-center justify-center rounded-xl bg-foreground p-3">
              <svg viewBox="0 0 33 33" className="h-full w-full" shapeRendering="crispEdges">
                ...
                <g fill="#000">
                  ...
                  {/* Top-right */}
                  <rect x="26" y="0" width="7" height="1"/><rect x="26" y="6" width="7" height="1"/>
                  <rect x="26" y="0" width="1" height="7"/><rect x="32" y="0" width="1" height="7"/>
                  <rect x="28" y="2" width="3" height="3"/>
                  {/* Bottom-left */}
                  <rect x="0" y="26" width="7" height="1"/><rect x="0" y="32" width="7" height="1"/>
                  <rect x="0" y="26" width="1" height="7"/><rect x="6" y="26" width="1" height="7"/>
                  ...
                  {/* Extra data near edges */}
                  <rect x="8" y="0" width="1" height="1"/><rect x="10" y="1" width="1" height="1"/>
                  <rect x="9" y="2" width="1" height="1"/><rect x="11" y="3" width="1" height="1"/>
                  <rect x="8" y="4" width="1" height="1"/><rect x="12" y="0" width="1" height="1"/>
                  <rect x="0" y="8" width="1" height="1"/><rect x="1" y="10" width="1" height="1"/>
                  <rect x="2" y="9" width="1" height="1"/><rect x="3" y="11" width="1" height="1"/>
                  <rect x="4" y="8" width="1" height="1"/><rect x="0" y="12" width="1" height="1"/>
                  <rect x="28" y="8" width="1" height="1"/><rect x="30" y="9" width="1" height="1"/>
                  <rect x="29" y="11" width="1" height="1"/><rect x="31" y="10" width="1" height="1"/>
                  <rect x="8" y="28" width="1" height="1"/><rect x="10" y="29" width="1" height="1"/>
                  <rect x="9" y="31" width="1" height="1"/><rect x="11" y="30" width="1" height="1"/>
                </g>
              </svg>
            </div>
      ...
      >
        ...
        {showMenu && (
          <div
            ref={menuRef}
            className="absolute bottom-full left-0 mb-2 w-52 rounded-2xl border border-border/15 bg-background/30 backdrop-blur-2xl shadow-[0_12px_48px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.08)] overflow-hidden"
            style={{ backgroundImage: "url('/ocean-bg.png')", backgroundSize: "cover", backgroundPosition: "center", backgroundBlendMode: "overlay" }}
          >
            <div className="flex flex-col p-2 bg-background/70 backdrop-blur-3xl rounded-2xl">
              <button
                onClick={() => { setShowSignIn(true); setShowMenu(false) }}
                className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold text-foreground transition-all duration-300 hover:bg-[#b0c4de]/15 hover:text-[#b0c4de] active:scale-95"
              >
        ...
        {/* Bar -- water glass translucent */}
        <div
          className="relative flex h-14 items-center gap-3 rounded-full border border-foreground/10 px-5 shadow-[0_8px_32px_rgba(0,0,0,0.3),0_2px_8px_rgba(0,0,0,0.2),inset_0_1px_0_rgba(255,255,255,0.1)] overflow-hidden"
        >
          ...
          <div
            ...
            aria-hidden="true"
          />
          <div className="absolute inset-0 bg-background/50 backdrop-blur-2xl" aria-hidden="true" />

          {/* Left: Menu */}
          <button
            onClick={() => setShowMenu(!showMenu)}
            className={cn(
              "relative z-10 flex items-center gap-2 rounded-full px-3 py-2 text-xs font-bold transition-all duration-300 active:scale-95",
              showMenu
                ? "bg-[#e6b800]/15 text-[#e6b800]"
                : "text-foreground/70 hover:text-[#b0c4de] hover:bg-foreground/5"
            )}
          >
          ...
          {/* Separator */}
          <div className="relative z-10 h-6 w-px bg-foreground/10" aria-hidden="true" />

          {/* Right: THALOS logo text (opens QR) */}
          <button
