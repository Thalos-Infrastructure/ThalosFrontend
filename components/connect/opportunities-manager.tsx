...
export function OpportunitiesManager({
  ...
  const changeStatus = async (opportunity: Opportunity, status: "open" | "closed" | "filled") => {
    ...
    toast.success(
      status === "filled"
        ? t("connect.opportunityFilled")
        : status === "closed"
          ? t("connect.opportunityClosed")
          : t("connect.opportunityCreated"),
    )
  }
  ...
  const setApplicationStatus = async (
    ...
    setApplicationsByOpp((prev) => ({
      ...prev,
      [opportunity.id]: (prev[opportunity.id] || []).map((a) =>
        a.id === application.id ? result.data! : a,
      ),
    }))
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      {formOpen && (
        <form
          ...
          className="mb-8 rounded-2xl border border-white/10 bg-[#0c1220] p-6 shadow-[0_8px_32px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.05)]"
        >
          <h3 className="mb-5 text-base font-semibold text-white">
            {t("connect.createOpportunity")}
          </h3>
          <div className="flex flex-col gap-4">
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-white/50">
                {t("connect.opportunityTitle")} <span className="text-rose-400">*</span>
              </label>
              <input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder={t("connect.opportunityTitlePlaceholder")}
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-white/50">
                {t("connect.opportunityDesc")}
              </label>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder={t("connect.opportunityDescPlaceholder")}
                rows={3}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  placeholder={t("connect.skillsPlaceholder")}
                  className={inputClass}
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-white/50">
                  {t("connect.budgetAmount")} <span className="text-rose-400">*</span>
                </label>
                ...
                {(["fixed", "milestone", "hourly"] as EngagementType[]).map((type) => (
                  <button
                    ...
                    className={cn(
                      ...
                      engagementType === type
                        ? "border-[#f0b400]/40 bg-[#f0b400]/5 text-[#f0b400]"
            ...
            <div className="flex justify-end">
              <Button
                type="submit"
                disabled={creating}
                className="rounded-full bg-[#f0b400] px-6 text-sm font-semibold text-background hover:bg-[#d4a000] disabled:opacity-40 shadow-[0_4px_16px_rgba(240,180,0,0.25)]"
              >
        ...
        <div className="flex flex-col gap-5">
          {opportunities.map((opportunity) => {
            ...
            return (
              <div
                key={opportunity.id}
                className="rounded-2xl border border-white/10 bg-[#0c1220] p-6 shadow-[0_8px_32px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.05)]"
              >
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="text-xl font-bold text-[#f0b400]">
                      ...
                      <span className="text-xs font-normal text-white/35">
                        {opportunity.budget_asset}
                      </span>
                    </span>
                    <span className="rounded-full bg-white/[0.04] px-3 py-1 text-xs font-medium text-white/50">
                      {t(`connect.engagement.${opportunity.engagement_type}`)}
                    </span>
                    {opportunity.skills_required.slice(0, 4).map((skill) => (
                      <span
                        key={skill}
                        className="rounded-full border border-white/[0.06] bg-white/[0.02] px-2.5 py-0.5 text-xs text-white/40"
                      >
                        {skill}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    {opportunity.status === "open" && (
                      <Button
                        variant="outline"
                        size="sm"
                        disabled={statusBusy === opportunity.id}
                ...
                {isExpanded && (
                  <div className="mt-5 border-t border-white/[0.06] pt-5">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-white/40">
                      {t("connect.applicants")} ({applications.length})
                    </p>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <p className="truncate font-mono text-[11px] text-white/30">
                                        {identity?.wallet
                                          ? `${identity.wallet.slice(0, 6)}...${identity.wallet.slice(-4)}`
                                          : application.builderId.slice(0, 8) + "..."}
                                      </p>
                                    </div>
                                    <span
                                      className={cn(
                              ...
                              <div className="mt-4 flex flex-wrap items-center gap-2">
                                {application.status === "pending" && (
                                  <>
                                    <Button
                                      size="sm"
                                      disabled={statusBusy === application.id}
                                      onClick={() =>
                                        setApplicationStatus(opportunity, application, "accepted")
                                      }
                                    ...
                                    <Button
                                      ...
                                      disabled={statusBusy === application.id}
                                      onClick={() =>
                                        setApplicationStatus(opportunity, application, "rejected")
                                      }
