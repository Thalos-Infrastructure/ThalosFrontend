...
export function ApproverAgreementDetail({ agr, walletAddress }: ApproverAgreementDetailProps) {
  ...
  async function signAndSubmit(
    ...
    res: AgreementResponse<unknown>,
    errorMessage: string,
  ): Promise<boolean> {
    if (!res.success) throw new Error(res.error || errorMessage)
    const xdr =
      res.data && typeof res.data === "object" && "unsignedTransaction" in res.data
  ...
  async function handleDispute(idx: number) {
    ...
    try {
      ...
      const res = await disputeMilestone(
        agr.id,
        idx.toString(),
        walletAddress,
        token ?? undefined,
    ...
    <div className="rounded-2xl border border-white/[0.06] bg-[#0a0a0c]/70 p-5 backdrop-blur-md">
      ...
      >
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-white/35">
            ...
            <span
              title={agr.id}
              className="max-w-[140px] overflow-hidden text-ellipsis whitespace-nowrap inline-block align-bottom"
            >
              {agr.id}
            </span>
            <span className="text-white/15">|</span>
        ...
        <div className="flex items-center gap-4 mt-2 sm:mt-0">
          ...
          <p className="text-lg font-bold text-white">
            ...
            {agr.milestones.length === 1
              ? agr.amount
              : agr.milestones.reduce((acc, ms) => acc + Number(ms.amount), 0)}
      ...
      <div className="mt-4 flex items-center gap-0">
        {[
          ...
          {
            label: t("flow.stepReview"),
            done: allApproved,
            active: currentStep === 1 || currentStep === 2,
          },
          {
            label: t("flow.stepRelease"),
            done: allReleased,
            active: currentStep === 2 && allApproved,
          },
        ].map((s, i) => (
          <React.Fragment key={i}>
            {i > 0 && (
              <div
                className={cn(
                  "h-px flex-1",
                  s.done || s.active ? "bg-[#f0b400]/40" : "bg-white/[0.06]",
                )}
              />
            )}
            <div
              className={cn(
                ...
                s.done
                  ...
                  : s.active
                    ? "bg-[#f0b400]/10 text-[#f0b400] border border-[#f0b400]/20"
              ...
              {s.done ? (
                ...
                <span
                  className={cn(
                    ...
                    s.active ? "bg-[#f0b400]" : "bg-white/20",
                  )}
                />
              )}
              {s.label}
      ...
      {fundSuccess && (
        <div className="text-emerald-400 text-xs mt-2">
          {t("flow.funded")} - Escrow funded successfully!
        </div>
      )}
              ...
              <React.Fragment key={s.key}>
                {i > 0 && (
                  <div
                    className={cn(
                      "h-px w-4",
                      done || active ? "bg-[#f0b400]/40" : "bg-white/[0.06]",
                    )}
                  />
                )}
                <div
                  ...
                  {done ? (
                    ...
                    <span
                      className={cn(
                        ...
                        active ? "bg-[#f0b400] animate-pulse" : "bg-white/20",
                      )}
                    />
                  )}
                  {s.label}
      ...
      {showDetail && (
        <div className="mt-4">
              ...
              <div
                ...
                className={cn(
                  ...
                  ms.status === "released"
                    ...
                    : ms.status === "approved" || ms.approved
                      ? "border-[#f0b400]/20 bg-[#f0b400]/5"
                  ...
                  <div>
                    ...
                    >
                      {ms.status === "released"
                        ? t("flow.released")
                        : ms.status === "approved" || ms.approved
                          ? t("flow.evidenceReady")
                          : isFunded
                            ? t("flow.awaitingEvidence")
                            : t("flow.pendingFundingDesc")}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <p className="text-lg font-bold text-white">
                    {"$"}
                    {localMilestones.length === 1 && idx === 0 ? agr.amount : ms.amount}{" "}
                  ...
                  {isFunded &&
                    (ms.status === "pending" || ms.status === "released") &&
                    !allReleased &&
                    ms.approved === false &&
                    !disputedMs.has(idx) &&
                    isExternalWallet && (
          ...
          {showDisputeConfirm !== null && (
            ...
            >
              <div
                className="w-full max-w-md rounded-2xl border border-red-500/20 bg-[#0c0c0e] p-6 shadow-[0_16px_48px_rgba(0,0,0,0.6)]"
            ...
            <div className="mt-4 rounded-xl border border-emerald-500/15 bg-emerald-500/5 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-white/35 mt-0.5">
                  All milestones are approved. Release funds to the seller.
                </p>
              </div>
              <Button
