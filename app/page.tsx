...
const ProfileSelection = dynamic(
  () => import("@/components/profile-selection").then((m) => ({ default: m.ProfileSelection })),
  { ssr: false },
)
...
const DashboardSection = dynamic(
  () => import("@/components/dashboard-section").then((m) => ({ default: m.DashboardSection })),
  { ssr: false },
)
...
const BottomBar = dynamic(
  () => import("@/components/bottom-bar").then((m) => ({ default: m.BottomBar })),
  { ssr: false },
)
...
export default function Home() {
  ...
  const setRef = useCallback(
    (section: string) => (el: HTMLDivElement | null) => {
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground">
      {/* Ocean collage background - higher quality and more visible */}
